...
const dadosFornecedor = async (req, res) => {
    ...
    const modeloID = resultUnidade[0].parFornecedorModeloID
    ...
    const [colunsFornecedor] = await db.promise().query(sqlFornecedor, [modeloID]);
    ...
    const sqlAllBlocks = `SELECT * FROM par_fornecedor_modelo_bloco a WHERE a.parFornecedorModeloID = ? `
    const [blocos] = await db.promise().query(sqlAllBlocks, [modeloID]);
    ...
    for (let i = 0; i < blocos.length; i++) {
        ...
        SELECT 
            ...
            (SELECT fr.obs 
                FROM fornecedor_resposta fr 
            WHERE fr.fornecedorID = ? AND fr.parFornecedorModeloBlocoID = a.parFornecedorModeloBlocoID AND fr.itemID = a.itemID) AS obsResposta
        FROM par_fornecedor_modelo_bloco_item a 
            JOIN item b ON (a.itemID = b.itemID)
        WHERE a.parFornecedorModeloBlocoID = ? AND a.status = 1
        ORDER BY a.ordem ASC`, [fornecedorID, fornecedorID, fornecedorID, blocos[i].parFornecedorModeloBlocoID]);

        resultBlocos.push({
            parFornecedorModeloBlocoID: blocos[i].parFornecedorModeloBlocoID,
            nome: blocos[i].nome,
            obs: blocos[i].obs,
            itens: resultTemp
        });
